        ...
        <div class="chatpage" ref="chatpage" @click="blurBotbar" @scroll="chatScroll">
            <van-list offset="0" v-model="loading" direction="up" :finished="finished" :finished-text="$t('没有更多了')"
                ...
                <div v-for="(item,index) in list" :key="item.id">
                    ...
                    <!-- 群成员变动通知 -->
                    <notice-change v-else-if="item.msg_type == 9" :message="item" :userId="selfInfo.id"></notice-change>
            ...
            <div v-if="receiveInfo.is_silence && receiveInfo.role>2">
                <div class="gap100"></div>
                <div class="safeArea"></div>
                <div class="chatSilenceBar">
                    <div class="gap100 chatCardBg flex jc ac size24 chatGray">{{ $t('已禁言！') }}</div>
                    <div class="safeArea"></div>
                </div>
            </div>
            <chat-botbar ref="botbar" :talkType="talk_type" :toId="receiveInfo.id"
                @show="scrollToBottom(true)" @blur="pageBlur" v-else></chat-botbar>
        </div>
